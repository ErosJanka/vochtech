...
<form method="GET" action="{{ route('reports.collaborators') }}" class="bg-white p-4 rounded shadow mb-4">
    <div class="grid grid-cols-3 gap-4">
        <div>
            ...
            <input type="text" name="name" value="{{ request('name') }}" class="w-full border p-2 rounded">
        </div>
        <div>
            <label class="block text-sm">CNPJ da Unidade</label>
            <input type="text" name="cnpj" value="{{ request('cnpj') }}" class="w-full border p-2 rounded">
        </div>
        ...
        <tbody>
            @foreach($collaborators as $c)
                <tr class="border-t">
                    ...
                    <td class="py-2">{{ $c->cpf }}</td>
                    <td class="py-2">{{ $c->unit->nome_fantasia ?? '-' }}</td>
                    <td class="py-2">{{ $c->unit->cnpj ?? '-' }}</td>
                </tr>
            @endforeach
        </tbody>
    </table>
